...
class MySQL(AppBase):
    ...
    def create_database(self, server, port, user, password, database, name, tables=None):
        try:
            conn = mysql.connector.connect(host=server, port=port, user=user, passwd=password)
            cursor = conn.cursor()
    ...
    # Create Tables
    def create_tables(self, server, port, user, password, database, tables):
        if not isinstance(tables, list) and not isinstance(tables, dict):
            tables = json.loads(tables)
    ...
    def insert_data(self, server, port, user, password, database, table, data):
        if not isinstance(data, list) and not isinstance(data, dict):
            data = json.loads(data)

        try:
            conn = mysql.connector.connect(
                host=server, port=port, user=user, passwd=password, db=database
            )
        except mysql.connector.Error as err:
            if err.errno == errorcode.ER_ACCESS_DENIED_ERROR:
                print("Something is wrong with your user name or password")
    ...
    # Query Data
    def query_data(
        self, server, port, user, password, database, table, fields=None, condition=None
    ):
    ...
    #update data    
    def update_data(self, server, port, user, password, database, table, fields=None, condition=None, data_value=None
    ):
    ...
    # delete data 
    def delete_data(self, server, port, user, password, database, table, fields=None, condition=None
    ):
    ...
    # join 
    def join(self, type, server, port, user, password, database, fields=None
    ):
        try:
            conn = mysql.connector.connect(
                host=server, port=port, user=user, passwd=password, db=database
